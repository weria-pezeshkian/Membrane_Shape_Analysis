...
def _track_blocks(
    ...
    residue. A `--select` block uses that selection string directly,
    tracked per residue, or as a single combined point when `select_whole`
    is set. `select_string` lets every downstream pass reselect a block's
    own atoms fresh from any `Universe` built on the same structure, in
    the same residue order every time.
    """
...
def _resolve_tracked_points(universe: mda.Universe, blocks: list[_TrackBlock]) -> np.ndarray:
    ...
    Row order is the concatenation of each block's own residues, in the
    order `block.select_string` selects them - the same order every later
    per-frame position array uses, so a roster row's index lines up with
    that row in every per-frame array throughout the run. A whole-selection
    block (`--select-whole`) contributes exactly one row, with `resindex`
    set to -1, marking that point as identified by its own block and index alone.
    """
    rows = []
    index = 0
    for block in blocks:
        ...
        for resindex in atoms.residues.resindices:
            rows.append((index, block.label, int(resindex), block.kind))
            index += 1
    dtype = [("index", "i8"), ("label", "U64"), ("resindex", "i8"), ("kind", "U16")]
...
def _block_positions(
    ...
    the same choice `analyze lipids` makes per species. A whole `select`
    block returns its entire matched atomgroup's own center of geometry as
    a single point; a per-residue `select` block returns one point per
    residue's own center of geometry.
    """
    atoms = universe.select_atoms(block.select_string)
    if block.kind == "lipids":
        if block.label in headgroup_override:
            xy, z, _ = _named_headgroup_centers(atoms, headgroup_override[block.label])
        else:
            xy, z, _ = _headgroup_centers(atoms, fourier_upper, fourier_lower)
        return xy, z
    if block.whole:
...
def _one_diffusion_frame(
    ...
    *,
    ...
    lower, 0 excluded). When `remove_tmd` is given, a hole mask is built
    fresh for this frame (`_remove_tmd_hole_mask`) and each tracked
    point's own hole status is looked up from it directly
    ...
    exact same surfaces without fitting them again.
    """
    diagnostics: list[tuple[str, str]] = []
    ...
    np.save(
        ...
        np.stack([fourier_upper.Anm, fourier_lower.Anm], axis=0),
    )
...
def _extract_whole_continuous_positions(
    ...
    `unwrap(tracked_ag)` keeps each tracked residue's own atoms from
    ...
    residue's own center of geometry for a `lipids` block), read from this
    ...
    for frame in frames:
        ...
        for block in blocks:
            ...
            if block.kind == "select" and block.whole:
                ...
                parts.append(atoms.center_of_geometry(compound="residues"))
...
def _build_segments(
    ...
    for i in range(len(tracked_points)):
        ...
        resindex = int(tracked_points["resindex"][i])
        for start, end in _break_into_segments(leaflet[:, i], in_hole[:, i]):
            ...
            segments.append({
                "label": label,
                "leaflet": "upper" if leaflet[start, i] == 1 else "lower",
                "resindex": resindex,
...
def _fit_diffusion_pools(
    ...
    for seg in segments:
        if seg["kept"]:
            pools[(seg["label"], seg["leaflet"])].append(seg["xy"])
            pools[(seg["label"], "both")].append(seg["xy"])
        else:
            discarded_by_label_leaflet[(seg["label"], seg["leaflet"])] += 1
            discarded_by_label[seg["label"]] += 1
...
def _write_diffusion_outputs(
    out_dir: str, segments: list[dict], diffusion_rows: list[tuple], msd_rows: list[tuple]
) -> None:
    """Write `diffusion.npy`, `msd_curves.npy`, and `segments.npy` as structured arrays."""
    diffusion_dtype = [
    ...
    msd_dtype = [("leaflet", "U8"), ("species", "U64"), ("tau_ps", "f8"), ("msd_A2", "f8"), ("n_samples", "i8")]
    segments_dtype = [
        ("label", "U64"), ("leaflet", "U8"), ("resindex", "i8"), ("start_frame", "i8"), ("end_frame", "i8"),
        ("length_frames", "i8"), ("length_fraction", "f8"), ("kept", "?"), ("discard_reason", "U64"),
    ]

    out = Path(out_dir)
    np.save(out / "diffusion.npy", np.array(diffusion_rows, dtype=diffusion_dtype))
    np.save(out / "msd_curves.npy", np.array(msd_rows, dtype=msd_dtype))
    np.save(
        out / "segments.npy",
        np.array(
            [
                (s["label"], s["leaflet"], s["resindex"], s["start_frame"], s["end_frame"],
...
def _finalize_diffusion(
    ...
    segments = _build_segments(tracked_points, leaflet, in_hole, proj_xy, frames, min_segment_fraction)
    diffusion_rows, msd_rows = _fit_diffusion_pools(
        segments, dt, max_tau_fraction, fit_tau_min_fraction, fit_tau_max_fraction
    )
    _write_diffusion_outputs(out_dir, segments, diffusion_rows, msd_rows)
...
def diffusion(args: list[str]) -> None:
    ...
    selection here (e.g. --Remove-TMD 'name BB SC1').
    """
    ...
    optional.add_argument(
        ...
        help="MDAnalysis selection to track, one point per residue unless --select-whole is given. "
