...
- `--min-balance` (default 0.6) - minimum acceptable leaflet-size balance
  ...
  atoms wins. 0.6 rejects splits more lopsided than roughly 4:1. See
  `core/leaflet.py`'s `get_components`/`_split_score`.
- `--margin` (default 2.0) - an atom is kept in a leaflet only if its
  ...
  sharp curvature, since curvature preserves inter-leaflet distance. See
  `core/leaflet.py`'s `apply_margin_filter`.
